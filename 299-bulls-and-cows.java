...
//     "bulls") and how many digits match the secret number but locate in the
...
public class Solution {
  public String getHint(String secret, String guess) {
    ...
    // get all
    int[] secretCounts = count(secret);
    int[] guessCounts = count(guess);
    int all = 0;
    for (int i=0; i<10; i++) {
      all += Math.min(secretCounts[i], guessCounts[i]);
    }
    // get cows
    int cows = all - bull;
